...
void Cmd_ExportScript_f( const idCmdArgs &args ) {
	...
	newProgram->EnableExport( args.Argc() > 1 ? args.Argv( 1 ) : NULL );
...
void idGameLocal::InitConsoleCommands( void ) {
	...
	cmdSystem->AddCommand( "exportScript",			Cmd_ExportScript_f,			CMD_FL_GAME|CMD_FL_CHEAT,	"exports scripts to C++" );
