...
ID_INLINE void CM_AddContact( cm_traceWork_t *tw ) {
	...
	if ( tw->modelCache.numContacts >= tw->modelCache.maxContacts ) {
		return;
	}
	// copy contact information from trace_t
	tw->modelCache.contacts[tw->modelCache.numContacts] = tw->trace.c;
	tw->modelCache.numContacts++;
...
================
*/
ID_INLINE void CM_SetVertexSidedness( cm_vertexSideCache_t &cache, const idPluecker &vpl, const idPluecker &epl, const int bitNum ) {
	if ( !(cache.sideSet & (1<<bitNum)) ) {
		float fl;
		fl = vpl.PermutedInnerProduct( epl );
		cache.side = (cache.side & ~(1<<bitNum)) | (FLOATSIGNBITSET(fl) << bitNum);
		cache.sideSet |= (1 << bitNum);
	}
}
...
================
*/
ID_INLINE void CM_SetEdgeSidedness( cm_edgeSideCache_t &cache, const idPluecker &vpl, const idPluecker &epl, const int bitNum ) {
	if ( !(cache.sideSet & (1<<bitNum)) ) {
		float fl;
		fl = vpl.PermutedInnerProduct( epl );
		cache.side = (cache.side & ~(1<<bitNum)) | (FLOATSIGNBITSET(fl) << bitNum);
...
void idCollisionModelManagerLocal::TranslateTrmEdgeThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *poly, cm_trmEdge_t *trmEdge ) {
	...
	for ( i = 0; i < poly->numEdges; i++) {
		...
		if ( tw->modelCache.edgeCache[ abs( edgeNum ) ].checkcount == tw->modelCache.checkCount ) {
			continue;
		}
		...
		cm_edgeSideCache_t &edgeSides = tw->modelCache.edgeSideCache[ abs( edgeNum ) ];
		CM_SetEdgeSidedness( edgeSides, *pl, tw->vertices[trmEdge->vertexNum[0]].pl, trmEdge->vertexNum[0] );
		CM_SetEdgeSidedness( edgeSides, *pl, tw->vertices[trmEdge->vertexNum[1]].pl, trmEdge->vertexNum[1] );
		// if the trm edge start and end vertex do not pass the polygon edge at different sides
		if ( !(((edgeSides.side >> trmEdge->vertexNum[0]) ^ (edgeSides.side >> trmEdge->vertexNum[1])) & 1) ) {
			continue;
		}
		// get the sides at which the polygon edge vertices pass the trm edge
		const int vertexNum1 = edge->vertexNum[INTSIGNBITSET(edgeNum)];
		const int vertexNum2 = edge->vertexNum[INTSIGNBITNOTSET(edgeNum)];
		v1 = tw->model->vertices + vertexNum1;
		CM_SetVertexSidedness( tw->modelCache.vertexSideCache[ vertexNum1 ], tw->polygonVertexPlueckerCache[i], trmEdge->pl, trmEdge->bitNum );
		v2 = tw->model->vertices + vertexNum2;
		CM_SetVertexSidedness( tw->modelCache.vertexSideCache[ vertexNum2 ], tw->polygonVertexPlueckerCache[i+1], trmEdge->pl, trmEdge->bitNum );
		// if the polygon edge start and end vertex do not pass the trm edge at different sides
		if ( !((tw->modelCache.vertexSideCache[ vertexNum1 ].side ^ tw->modelCache.vertexSideCache[ vertexNum2 ].side) & (1<<trmEdge->bitNum)) ) {
			continue;
		}
		...
		if ( f2 < tw->trace.fraction ) {
			...
			tw->trace.c.surfaceType = idCollisionModelManagerLocal::GetSurfaceType( &tw->trace.c, poly, &tw->trace.c.surfaceColor );
...
void idCollisionModelManagerLocal::TranslateTrmVertexThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *poly, cm_trmVertex_t *v, int bitNum ) {
	...
	if ( f < tw->trace.fraction ) {
		...
		for ( i = 0; i < poly->numEdges; i++ ) {
			...
			edge = tw->model->edges + abs(edgeNum);
			cm_edgeSideCache_t &edgeSides = tw->modelCache.edgeSideCache[ abs( edgeNum ) ];
			CM_SetEdgeSidedness( edgeSides, tw->polygonEdgePlueckerCache[i], v->pl, bitNum );
			if ( INTSIGNBITSET(edgeNum) ^ ((edgeSides.side >> bitNum) & 1) ) {
				return;
			}
		}
		if ( f < 0.0f ) {
		...
		tw->trace.c.modelFeature = static_cast< int >( poly - tw->model->polygons );
...
void idCollisionModelManagerLocal::TranslatePointThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *poly, cm_trmVertex_t *v ) {
	...
	if ( f < tw->trace.fraction ) {
		...
		for ( i = 0; i < poly->numEdges; i++ ) {
			...
			cm_edgeCache_t &edgeVisit = tw->modelCache.edgeCache[ abs( edgeNum ) ];
			cm_edgeSideCache_t &edgeSides = tw->modelCache.edgeSideCache[ abs( edgeNum ) ];
			if ( edgeVisit.checkcount != tw->modelCache.checkCount ) {
				...
				edgeSides.side = FLOATSIGNBITSET(fl);
			}
			// if the point passes the edge at the wrong side
			//if ( (edgeNum > 0) == edge->side ) {
			if ( INTSIGNBITSET(edgeNum) ^ edgeSides.side ) {
				return;
			}
		}
		if ( f < 0.0f ) {
...
void idCollisionModelManagerLocal::TranslateVertexThroughTrmPolygon( cm_traceWork_t *tw, cm_trmPolygon_t *trmpoly, cm_polygon_t *poly, cm_vertex_t *v, idVec3 &endp, idPluecker &pl ) {
	...
	if ( f < tw->trace.fraction ) {
		...
		for ( i = 0; i < trmpoly->numEdges; i++ ) {
			...
			const int vertexNum = static_cast< int >( v - tw->model->vertices );
			cm_vertexSideCache_t &vertexSides = tw->modelCache.vertexSideCache[ vertexNum ];
			CM_SetVertexSidedness( vertexSides, pl, edge->pl, edge->bitNum );
			if ( INTSIGNBITSET(edgeNum) ^ ((vertexSides.side >> edge->bitNum) & 1) ) {
				return;
			}
		}
		if ( f < 0.0f ) {
...
bool idCollisionModelManagerLocal::TranslateTrmThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *p ) {
	...
	cm_vertex_t *v;
	cm_edge_t *e;
	const int polygonNum = static_cast< int >( p - tw->model->polygons );
	...
	if ( tw->pointTrace ) {
		...
		for ( i = 0; i < p->numEdges; i++ ) {
			...
			if ( tw->modelCache.edgeCache[ abs( edgeNum ) ].checkcount != tw->modelCache.checkCount ) {
				tw->modelCache.edgeSideCache[ abs( edgeNum ) ].sideSet = 0;
			}
			...
			const int polygonVertexNum = e->vertexNum[INTSIGNBITSET(edgeNum)];
			v = &tw->model->vertices[ polygonVertexNum ];
			// reset sidedness cache if this is the first time we encounter this vertex during this trace
			if ( tw->modelCache.vertexCache[ polygonVertexNum ].checkcount != tw->modelCache.checkCount ) {
				tw->modelCache.vertexSideCache[ polygonVertexNum ].sideSet = 0;
			}
		...
		for ( i = 0; i < p->numEdges; i++ ) {
			...
			for ( k = 0; k < 2; k++ ) {
				...
				const int vertexNum = e->vertexNum[k ^ INTSIGNBITSET(edgeNum)];
				v = tw->model->vertices + vertexNum;
...
bool idCollisionModelManagerLocal::TranslatePointTrmThroughPolygon( idTraceWork *tw, int polygonNum ) {
	return TranslateTrmThroughPolygon( tw, &tw->model->polygons[ polygonNum ] );
}

bool idCollisionModelManagerLocal::TranslateTrmThroughPolygon( idTraceWork *tw, int polygonNum ) {
	return TranslateTrmThroughPolygon( tw, &tw->model->polygons[ polygonNum ] );
}
...
void idCollisionModelManagerLocal::Translation( trace_t *results, const idVec3 &start, const idVec3 &end,
	...
	cm_trmEdge_t *edge;
	cm_trmVertex_t *vert;
	idTraceWork *threadWork = traceWork[ GetThreadId() ];
	if ( threadWork == NULL ) {
		threadWork = baseTraceWork;
	}
	idTraceWork &tw = *threadWork;
	...
	cm_model_t *collisionModel = static_cast< cm_model_t * >( model );
	...
	tw.modelCache.UpdateForModel( collisionModel );
	...
	tw.quickExit = false;
	tw.getContacts = tw.modelCache.contacts != NULL;
	tw.contacts = tw.modelCache.contacts;
	tw.maxContacts = tw.modelCache.maxContacts;
	tw.numContacts = 0;
	tw.modelCache.numContacts = 0;
	tw.model = collisionModel;
	...
	if ( !trm || ( trm->bounds[1][0] - trm->bounds[0][0] <= 0.0f &&
		...
		tw.trmExtents = tw.extents;
		tw.size.Zero();
		tw.trmBounds.Zero();
		tw.traceBounds.SetBounds( tw.bounds );
		...
		*results = tw.trace;
		...
		tw.numContacts = tw.modelCache.numContacts;
		idCollisionModelManagerLocal::numContacts = tw.modelCache.numContacts;
		return;
	}
	...
	tw.trmBounds.SetBounds( tw.size );
	tw.traceBounds.SetBounds( tw.bounds );
	tw.trmExtents = tw.extents;
	tw.traceType = 1;
	...
	if ( tw.getContacts ) {
		...
		if ( model_rotated ) {
			for ( i = 0; i < tw.modelCache.numContacts; i++ ) {
...
void idCollisionModelManagerLocal::Translation( trace_t *results, const idVec3 &start, const idVec3 &end,
	...
	if ( model < 0 || model >= numModels || models == NULL ) {
		memset( results, 0, sizeof( *results ) );
		common->Warning( "idCollisionModelManagerLocal::Translation: invalid model handle %d", model );
		return;
	}
	Translation( results, start, end, trm, trmAxis, contentMask,
		models[ model ], modelOrigin, modelAxis );
}
