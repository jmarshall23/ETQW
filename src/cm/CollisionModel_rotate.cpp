...
void idCollisionModelManagerLocal::RotateTrmEdgeThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *poly, cm_trmEdge_t *trmEdge ) {
	...
	if ( !poly->bounds.IntersectsBounds( trmEdge->rotationBounds ) ) {
		return;
	}
...
void idCollisionModelManagerLocal::RotateTrmVertexThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *poly, cm_trmVertex_t *v, int vertexNum ) {
	...
	if ( !poly->bounds.IntersectsBounds( v->rotationBounds ) ) {
		return;
	}
...
bool idCollisionModelManagerLocal::RotateTrmThroughPolygon( cm_traceWork_t *tw, cm_polygon_t *p ) {
	...
	if ( !p->bounds.IntersectsBounds( tw->traceBounds ) ) {
		return false;
	}
	...
	switch( tw->traceBounds.PlaneSide( p->plane ) ) {
	...
	if ( tw->pointTrace ) {
		...
		for ( i = 0; i < p->numEdges; i++ ) {
			...
			tw->modelCache.edgeCache[ abs( edgeNum ) ].checkcount = tw->modelCache.checkCount;
			// can never collide with internal edges
			if ( e->internal ) {
				continue;
			}
			...
			for ( k = 0; k < 2; k++ ) {
				...
				tw->modelCache.vertexCache[ vertexNum ].checkcount = tw->modelCache.checkCount;

				// if the vertex is outside the trm rotation bounds
				if ( !tw->traceBounds.ContainsPoint( v->p ) ) {
					continue;
				}
...
ETQW stores polygon references as array indices.  Keep the geometry helper
pointer-based while exposing the retail index-based dispatch used by the BSP.
================
*/
bool idCollisionModelManagerLocal::RotateTrmThroughPolygon( idTraceWork *tw, int polygonNum ) {
	if ( polygonNum < 0 || polygonNum >= tw->model->numPolygons ) {
		return false;
	}
	return RotateTrmThroughPolygon( tw, &tw->model->polygons[ polygonNum ] );
}
...
void idCollisionModelManagerLocal::Rotation( trace_t *results, const idVec3 &start, const idRotation &rotation,
	...
	if ( model < 0 || model >= numModels || models == NULL ) {
		...
		results->endpos = start;
		results->endAxis = trmAxis * rotation.ToMat3();
		common->Warning( "idCollisionModelManagerLocal::Rotation: invalid model handle %d", model );
		return;
	}
	Rotation( results, start, rotation, trm, trmAxis, contentMask,
