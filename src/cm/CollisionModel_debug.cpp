...
int cm_contentsFlagByIndex[] = {
	...
	CONTENTS_CORPSE,				// 10
	CONTENTS_TRIGGER,				// 11
	CONTENTS_AAS_SOLID_PLAYER,		// 12
	CONTENTS_AAS_OBSTACLE,			// 13
	CONTENTS_AAS_SOLID_VEHICLE,		// 14
	0
};
...
================
*/
int idCollisionModelManagerLocal::ContentsFromString( const char *string, int startIndex ) const {
	int i, contents = 0;
	if ( string == NULL ) {
		return 0;
	}
	...
	while( src.ReadToken( &token ) ) {
		...
		for ( i = startIndex; cm_contentsNameByIndex[i] != NULL; i++ ) {
...
void idCollisionModelManagerLocal::DrawEdge( cm_model_t *model, int edgeNum, const idVec3 &origin, const idMat3 &axis,
	...
	if ( edge->internal ) {
		if ( cm_drawInternal.GetBool() ) {
			session->rw->DebugLine( colorGreen, start, end, lifetime );
		}
	} else {
		if ( edge->numUsers > 2 ) {
			session->rw->DebugLine( colorBlue, start, end, lifetime );
		} else {
			session->rw->DebugLine( cm_color, start, end, lifetime );
		}
	}
	...
	if ( cm_drawNormals.GetBool() ) {
		...
		session->rw->DebugLine( colorCyan, mid, end, lifetime );
	}
	(void)viewOrigin;
	(void)viewAxis;
	(void)radius;
...
================
*/
void idCollisionModelManagerLocal::DrawPolygon( cm_model_t *model, cm_polygon_t *p, const idVec3 &origin, const idMat3 &axis,
		const idVec3 &viewOrigin, const idMat3 &viewAxis, float radius, int lifetime ) {
	int i, edgeNum;
	cm_edge_t *edge;
	...
	if ( cm_drawNormals.GetBool() ) {
		...
		session->rw->DebugArrow( colorMagenta, center, end, 1, lifetime );
	}
	...
	if ( cm_drawFilled.GetBool() ) {
		...
		session->rw->DebugPolygon( cm_color, winding, lifetime );
	} else {
		for ( i = 0; i < p->numEdges; i++ ) {
			edgeNum = p->edges[i];
			edge = model->edges + abs(edgeNum);
			cm_edgeCache_t &edgeVisit = baseTraceWork->modelCache.edgeCache[ abs( edgeNum ) ];
			if ( edgeVisit.checkcount == baseTraceWork->modelCache.checkCount ) {
				continue;
			}
			edgeVisit.checkcount = baseTraceWork->modelCache.checkCount;
			DrawEdge( model, edgeNum, origin, axis, viewOrigin, viewAxis, radius, lifetime );
		}
	}
}
...
void idCollisionModelManagerLocal::DrawNodePolygons( cm_model_t *model, cm_node_t *node,
	...
	while ( node != NULL ) {
		for ( pref = node->polygons; pref; pref = pref->next ) {
			if ( pref->polygonNum < 0 || pref->polygonNum >= model->numPolygons ) {
				continue;
			}
			p = &model->polygons[ pref->polygonNum ];
			if ( radius ) {
			...
			cm_polygonCache_t &polygonVisit = baseTraceWork->modelCache.polygonCache[ pref->polygonNum ];
			if ( polygonVisit.checkcount == baseTraceWork->modelCache.checkCount ) {
				continue;
			}
			if ( !( p->contents & ContentsFromString( cm_drawMask.GetString(), 0 ) ) ) {
				continue;
			}

			DrawPolygon( model, p, origin, axis, viewOrigin, viewAxis, radius, lifetime );
			polygonVisit.checkcount = baseTraceWork->modelCache.checkCount;
		}
		if ( node->planeType == -1 ) {
			break;
		}
		if ( radius && viewOrigin[node->planeType] > node->planeDist + radius  ) {
			...
			DrawNodePolygons( model, node->children[1], origin, axis, viewOrigin, viewAxis, radius, lifetime );
			node = node->children[0];
		}
	}
}
...
================
*/
void idCollisionModelManagerLocal::DrawModel( idCollisionModel *collisionModel, const idVec3 &modelOrigin,
					const idMat3 &modelAxis, const idVec3 &viewOrigin, const idMat3 &viewAxis,
					const float radius, int lifetime ) {

	cm_model_t *model;
	idVec3 viewPos;

	if ( collisionModel == NULL || baseTraceWork == NULL ) {
		return;
	}
	...
	model = static_cast< cm_model_t * >( collisionModel );
	...
	DrawNodePolygons( model, model->node, modelOrigin, modelAxis, viewPos, viewAxis, radius, lifetime );
}

void idCollisionModelManagerLocal::DrawModel( cmHandle_t handle, const idVec3 &modelOrigin, const idMat3 &modelAxis,
		const idVec3 &viewOrigin, const float radius ) {
	if ( handle < 0 || handle >= numModels || models == NULL ) {
		return;
	}
	DrawModel( models[ handle ], modelOrigin, modelAxis, viewOrigin, mat3_identity, radius, 0 );
}
