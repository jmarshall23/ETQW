...
void Dmap( const idCmdArgs &args ) {
	...
	if ( !leaked ) {
		...
		if ( !noCM ) {
			// The reconstructed ETQW collision manager currently exposes only
			// runtime .cm loading.  Keep dmap's geometry/procb output usable and
			// make the missing compiler stage explicit instead of calling the
			// incompatible Doom 3 collision compiler API.
			common->Warning( "dmap: ETQW collision-map generation is not available yet; use -noCM to silence this warning" );
		}
