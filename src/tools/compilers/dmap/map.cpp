...
static bool Dmap_IsIdentifierCharacter( char c ) {
	return ( c >= 'a' && c <= 'z' ) || ( c >= 'A' && c <= 'Z' ) ||
		( c >= '0' && c <= '9' ) || c == '_';
}

static void Dmap_LoadMapScriptText( void ) {
	idStr call;
	if ( dmapGlobals.dmapFile && dmapGlobals.dmapFile->GetNumEntities() > 0 ) {
		call = dmapGlobals.dmapFile->GetEntity( 0 )->epairs.GetString( "call", "" );
	}
	if ( call == dmapStaticScriptCall ) {
		return;
	}

	dmapStaticScriptCall = call;
	dmapStaticScriptText.Clear();
	const int namespaceEnd = call.Find( "::" );
	if ( namespaceEnd <= 0 ) {
		return;
	}

	idStr scriptNamespace;
	call.Left( namespaceEnd, scriptNamespace );
	idStr scriptName = "script/" + scriptNamespace + ".script";
	void *scriptBuffer = NULL;
	const int scriptLength = fileSystem->ReadFile( scriptName, &scriptBuffer, NULL );
	if ( scriptLength > 0 && scriptBuffer ) {
		dmapStaticScriptText = (const char *)scriptBuffer;
		fileSystem->FreeFile( scriptBuffer );
		common->Printf( "Static lightmap classification scanning %s\n", scriptName.c_str() );
	}
}

static bool Dmap_MapScriptMentionsEntity( const char *entityName ) {
	if ( !entityName || !entityName[0] ) {
		return false;
	}
	Dmap_LoadMapScriptText();
	const char *text = dmapStaticScriptText.c_str();
	const int nameLength = idStr::Length( entityName );
	for ( const char *match = strstr( text, entityName ); match; match = strstr( match + nameLength, entityName ) ) {
		const bool validStart = match == text || !Dmap_IsIdentifierCharacter( match[-1] );
		const bool validEnd = !Dmap_IsIdentifierCharacter( match[nameLength] );
...
static bool Dmap_HasIncomingTarget( const char *entityName ) {
	if ( !dmapGlobals.dmapFile || !entityName || !entityName[0] ) {
		return false;
	}
	for ( int entityNum = 0; entityNum < dmapGlobals.dmapFile->GetNumEntities(); entityNum++ ) {
		const idDict &args = dmapGlobals.dmapFile->GetEntity( entityNum )->epairs;
		for ( int keyNum = 0; keyNum < args.GetNumKeyVals(); keyNum++ ) {
			const idKeyValue *keyValue = args.GetKeyVal( keyNum );
			if ( !keyValue->GetKey().Icmpn( "target", 6 ) && !keyValue->GetValue().Icmp( entityName ) ) {
				return true;
			}
		}
	}
...
static bool Dmap_HasChangeableStaticSpawnArgs( const idDict &args ) {
	if ( args.GetString( "bind", "" )[0] || args.GetString( "scriptobject", "" )[0] ||
		args.GetString( "call", "" )[0] || args.GetBool( "hide", "0" ) ||
		args.GetBool( "start_off", "0" ) || args.GetBool( "runGui", "0" ) ||
		args.GetInt( "health", "0" ) > 0 || args.GetBool( "break", "0" ) ) {
		return true;
	}
	if ( args.MatchPrefix( "gui", NULL ) || args.MatchPrefix( "target", NULL ) ||
		args.MatchPrefix( "shaderParm", NULL ) || args.FindKey( "_color" ) ) {
		return true;
	}
	return false;
}

bool Dmap_IsStaticEntityImmutable( const idMapEntity *mapEntity ) {
	if ( !mapEntity ) {
		return false;
	}
	...
	if ( bakeOverride ) {
		return args.GetBool( "bakeLightmap", "0" );
	}
	const char *name = args.GetString( "name", "" );
	return !Dmap_HasChangeableStaticSpawnArgs( args ) &&
		!Dmap_MapScriptMentionsEntity( name ) && !Dmap_HasIncomingTarget( name );
}

bool Dmap_ShouldInlineStatic( const idMapEntity *mapEntity ) {
	if ( !mapEntity ) {
		return false;
	}
	const idDict &args = mapEntity->epairs;
	const bool inlineAll = dmapGlobals.dmapFile && dmapGlobals.dmapFile->GetNumEntities() > 0 &&
		dmapGlobals.dmapFile->GetEntity( 0 )->epairs.GetBool( "inlineAllStatics", "0" );
	if ( args.GetBool( "inline", "0" ) || inlineAll ) {
		return true;
	}
	if ( !Dmap_IsStaticEntityImmutable( mapEntity ) || mapEntity->GetNumPrimitives() != 0 ) {
		return false;
	}
	const char *modelName = args.GetString( "model", "" );
	const char *entityName = args.GetString( "name", "" );
	return modelName[0] && idStr::Icmp( modelName, entityName );
}
...
static void ParseBrush( const idMapBrush *mapBrush, int primitiveNum ) {
	...
	for ( i = 0 ; i < mapBrush->GetNumSides() ; i++ ) {
		...
		s->material = declHolder.FindMaterial( ms->GetMaterial() );
...
static void ParsePatch( const idMapPatch *patch, int primitiveNum ) {
	...
	mat = declHolder.FindMaterial( patch->GetMaterial() );
