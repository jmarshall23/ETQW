...
static void MakeHeadnodePortals( tree_t *tree ) {
	...
	for (i=0 ; i<3 ; i++) {
		for (j=0 ; j<2 ; j++) {
			...
			p->plane = *pl;
			p->winding = DmapWindingForPlane( *pl );
	...
	for (i=0 ; i<6 ; i++) {
		for (j=0 ; j<6 ; j++) {
			...
			portals[i]->winding = DmapClipWinding( portals[i]->winding, bplanes[j], ON_EPSILON );
		}
	}
}
...
idWinding *BaseWindingForNode (node_t *node) {
	...
	w = DmapWindingForPlane( dmapGlobals.mapPlanes[node->planenum] );
	...
	for ( n = node->parent ; n && w ; ) {
		...
		if ( n->children[0] == node ) {
			// take front
			w = DmapClipWinding( w, plane, BASE_WINDING_EPSILON );
		} else {
			// take back
			idPlane	back = -plane;
			w = DmapClipWinding( w, back, BASE_WINDING_EPSILON );
		}
		node = n;
		n = n->parent;
	}
...
static void MakeNodePortal( node_t *node ) {
	...
	for (p = node->portals ; p && w; p = p->next[side])	
	{
		...
		w = DmapClipWinding( w, plane, CLIP_EPSILON );
	}
...
bool FloodEntities( tree_t *tree ) {
	...
	for (i=1 ; i<dmapGlobals.num_entities ; i++) {
		...
		if ( !strcmp( cl, "light" ) ) {
			...
			if ( v[0] ) {
				const idMaterial *mat = declHolder.FindMaterial( v );
				if ( mat->IsFogLight() ) {
					continue;
				}
			}
