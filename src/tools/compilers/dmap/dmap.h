...
// dmap only needs the renderer's public mesh representation plus a small set
// of triangle utility functions.  Pulling in tr_local.h binds the tool to the
// Doom 3 renderer internals and conflicts with ETQW's renderer declarations.
const idMaterial *R_RemapShaderBySkin( const idMaterial *shader, const idDeclSkin *customSkin, const idMaterial *customShader );

ID_INLINE idWinding *DmapWindingForPlane( const idPlane &plane ) {
	return new idWinding( plane, MAX_WORLD_COORD );
}

ID_INLINE idWinding *DmapClipWinding( idWinding *winding, const idPlane &plane, float epsilon ) {
	if ( winding != NULL && !winding->ClipInPlace( plane, epsilon ) ) {
		delete winding;
		return NULL;
	}
	return winding;
...
// The original compiler code uses the classic three-component vector macros.
// ETQW's idlib deliberately does not export them, so keep compatibility local
// to dmap rather than changing the engine math API.
#ifndef DotProduct
...
#endif
#ifndef VectorMA
#define VectorMA( a, scale, b, c ) do { ( c )[ 0 ] = ( a )[ 0 ] + ( scale ) * ( b )[ 0 ]; ( c )[ 1 ] = ( a )[ 1 ] + ( scale ) * ( b )[ 1 ]; ( c )[ 2 ] = ( a )[ 2 ] + ( scale ) * ( b )[ 2 ]; } while ( 0 )
#endif
