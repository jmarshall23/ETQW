...
bool CreateBrushWindings (uBrush_t *brush) {
	...
	for ( i = 0; i < brush->numsides; i++ ) {
		...
		for ( j = 0; j < brush->numsides && w; j++ ) {
			...
			w = DmapClipWinding( w, *plane, 0 );//CLIP_EPSILON);
		}
		if ( side->winding ) {
			delete side->winding;
		}
...
void WriteBspBrushMap( const char *name, uBrush_t *list ) {
	...
	for ( ; list ; list=list->next )
	{
		...
		{
			w = DmapWindingForPlane( dmapGlobals.mapPlanes[s->planenum] );
...
void SplitBrush (uBrush_t *brush, int planenum, uBrush_t **front, uBrush_t **back) {
	...
	*front = *back = NULL;
	...
	for ( i = 0; i < brush->numsides && w; i++ ) {
		...
		w = DmapClipWinding( w, plane2, 0 ); // PLANESIDE_EPSILON);
	}
