...
void		TriVertsFromOriginal( mapTri_t *tri, const mapTri_t *original ) {
	...
	for ( i = 0 ; i < 3 ; i++ ) {
		...
		const idVec2 st0 = original->v[0].GetST();
		const idVec2 st1 = original->v[1].GetST();
		const idVec2 st2 = original->v[2].GetST();
		tri->v[i].SetST( st0 * a + st1 * b + st2 * c );

		idVec3 normal = original->v[0].GetNormal() * a
			+ original->v[1].GetNormal() * b + original->v[2].GetNormal() * c;
		normal.Normalize();
