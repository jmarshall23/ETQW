...
void RadiantInit( void ) {
	...
	if ( !renderSystem->IsOpenGLRunning() ) {
		common->Printf( "no renderer running\n" );
		return;
	}
	if ( !R_UseVulkanBackend() ) {
		common->Printf( "ETQW ImGui Radiant requires the Vulkan renderer. Start with +set r_renderAPI vulkan +editor.\n" );
		return;
	}
	...
	if ( g_DoomInstance ) {
		...
		if ( RadiantImGuiEnabled() ) {
			if ( RadiantImGuiCreate() ) {
				...
				common->Warning( "Could not create the Vulkan Dear ImGui Radiant window.\n" );
			}
		}
...
void RadiantOpenMap( const char *mapName ) {
