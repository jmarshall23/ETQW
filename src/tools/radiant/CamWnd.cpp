...
void CCamWnd::Cam_BuildMatrix() {
	...
	xa = ((renderMode && !R_UseVulkanBackend()) ? -m_Camera.angles[PITCH] : m_Camera.angles[PITCH]) * idMath::M_DEG2RAD;
...
void CCamWnd::Cam_Draw() {
	...
	// float yfov;
	int		i;
	const bool renderWorldMode = renderMode && !R_UseVulkanBackend();
	...
	g_qeglobals.sw->PlaceListener( m_Camera.origin, axis, 0, Sys_Milliseconds() );
...
void CCamWnd::BuildEntityRenderState( entity_t *ent, bool update) {
	...
	if ( !update ) {
		...
		if ( ent->modelDef >= 0 ) {
			renderEntity_t *oldEntity = g_qeglobals.rw->GetRenderEntity( ent->modelDef );
			if ( oldEntity != NULL && oldEntity->joints != NULL ) {
				Mem_Free( oldEntity->joints );
				oldEntity->joints = NULL;
			}
	...
	if ( v && *v ) {
		...
		if ( !stricmp( name, v ) ) {
			...
			refent.hModel = bmodel;

		} else {
			// use the game's epair parsing code so
			// we can use the same renderEntity generation
			gameEdit->ParseSpawnArgsToRenderEntity( spawnArgs, refent );
			const int numJoints = refent.hModel != NULL ? refent.hModel->NumJoints() : 0;
			const idMD5Anim *anim = gameEdit->ANIM_GetAnimFromEntityDef( &spawnArgs );
			if ( numJoints > 0 && anim != NULL ) {
				refent.numJoints = numJoints;
				refent.joints = ( idJointMat * )Mem_Alloc( numJoints * sizeof( *refent.joints ) );
				int frame = spawnArgs.GetInt("frame") + 1;
				if ( frame < 1 ) {
					frame = 1;
				}
				const idVec3 &offset = gameEdit->ANIM_GetModelOffsetFromEntityDef( &spawnArgs );
				gameEdit->ANIM_CreateAnimFrame( refent.hModel, anim, numJoints, refent.joints, ( frame * 1000 ) / 24, offset, false );
			}
		}

		refent.referenceSound = ent->soundEmitter;
		if ( ent->modelDef >= 0 ) {
			g_qeglobals.rw->UpdateEntityDef( ent->modelDef, &refent );
		} else {
...
void Tris_ToOBJ(const char *outFile, idTriList *tris, idMatList *mats) {
	...
	if ( f ) {
		...
		for (i = 0; i < tris->Num(); i++) {
			...
			for (j = 0; j < tri->numVerts; j++) {
				const idVec2 st = tri->verts[j].GetST();
				f->Printf( "vt %f %f\n", st.x, 1.0f - st.y );
			}
			for (j = 0; j < tri->numVerts; j++) {
	...
	if ( brush->pPatch ) {
		...
		for ( i = 0; i < pm->width; i++ ) {
			for ( j = 0; j < pm->height; j++ ) {
				...
				(*cp)[j*cp->GetWidth()+i].SetST( pm->ctrl(i, j) );
			}
		}
...
void Select_ToCM() {
	...
	if ( dlgFile.DoModal() == IDOK ) {
		...
		// ETQW collision data is generated by dmap, not serialized by Radiant.
...
bool CCamWnd::UpdateRenderEntities() {
	...
	for ( entity_t *ent = entities.next ; ent != &entities ; ent = ent->next ) {
		BuildEntityRenderState( ent, (ent->lightDef >= 0 || ent->modelDef >= 0 || ent->soundEmitter ) ? true : false );
...
void CCamWnd::FreeRendererState() {
	...
	for ( entity_t *ent = entities.next ; ent != &entities ; ent = ent->next ) {
		...
		if (ent->modelDef >= 0) {
			renderEntity_t *refent = g_qeglobals.rw->GetRenderEntity( ent->modelDef );
			if ( refent != NULL && refent->joints != NULL ) {
				Mem_Free( refent->joints );
				refent->joints = NULL;
			}
			g_qeglobals.rw->FreeEntityDef( ent->modelDef );
			ent->modelDef = -1;
