...
const int		g_msgBSPDone = RegisterWindowMessage( "DMAPDone" );
const int		g_msgBSPStatus = RegisterWindowMessage( "DMAPOutput" );
...
void CMainFrame::OnFileSave() {
	if (!strcmp(currentmap, "unnamed.world")) {
		SaveAsDialog(false);
	}
	else {
...
void CMainFrame::OnFileSaveCopy() {
	char aFile[260] = "\0";
	char aFilter[260] = "ETQW World\0*.world\0\0";
	char aTitle[260] = "Save a Copy\0";
	OPENFILENAME afn;

	memset( &afn, 0, sizeof(OPENFILENAME) );

	CString strPath = ValueForKey(g_qeglobals.d_project_entity, "mapspath");
	if ( strPath.IsEmpty() ) {
		strPath = ValueForKey(g_qeglobals.d_project_entity, "basepath");
		AddSlash(strPath);
		strPath += "maps";
	}
	...
	DefaultExtension(afn.lpstrFile, ".world");
...
void RunBsp (const char *command) {
	...
	// decide if we are compiling the .world or a temporary .reg
...
void CMainFrame::OnBspCommand(unsigned int nID) {
	if (g_PrefsDlg.m_bSnapShots && stricmp(currentmap, "unnamed.world") != 0) {
		Map_Snapshot();
	}
...
void CMainFrame::OnFileImportmap() {
	CFileDialog dlgFile(TRUE, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "ETQW World files (*.world)|*.world||", this);
...
void CMainFrame::OnFileExportmap() {
	CFileDialog dlgFile(FALSE, "world", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "ETQW World files (*.world)|*.world||", this);
	if (dlgFile.DoModal() == IDOK) {
...
void CMainFrame::OnGenerateMaterialsList()
{
	...
	for ( brush_t* b = active_brushes.next ; b != &active_brushes ; b=b->next ) {
		if ( b->pPatch ){
			...
			if ( mtrList.FindIndex( mtrName ) < 0 ) {
...
void CMainFrame::OnDoomScriptBlueprintEditor() {
	common->Printf( "DoomScript Blueprint Editor is not part of the ETQW tool port.\n" );
}

void CMainFrame::OnGUIEditor() {
	common->Printf( "GUI Editor is not part of the ETQW tool port.\n" );
}
