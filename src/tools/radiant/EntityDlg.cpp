...
void CEntityDlg::OnCbnAnimationChange ()
{
	...
	if ( sel != -1 ) {
		...
		if ( animName.GetLength() > 0 ) {
			...
			//update the slider
			idDict animArgs = editEntity->epairs;
			animArgs.Set( "classname", editEntity->eclass->name );
			animArgs.Set( "anim", animName.GetBuffer(0) );
			currentAnimation = gameEdit->ANIM_GetAnimFromEntityDef( &animArgs );
