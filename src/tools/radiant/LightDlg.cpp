...
void CLightDlg::SaveLightInfo( const idDict *differences ) {
	...
	if ( com_editorActive ) {
		...
		list.SetNum( 128 );
		int count = gameEdit->GetSelectedEntities( list.Begin(), list.Num() );
		list.SetNum( count );
...
void LightEditorInit( const idDict *spawnArgs ) {
	...
	common->Warning( "The Vulkan ImGui Radiant shell could not be created; the light editor is unavailable.\n" );
}
