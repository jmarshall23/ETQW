...
BOOL CInspectorDialog::OnInitDialog()
{
	...
	if ( RadiantImGuiEnabled() ) {
		// These dialogs are state/controllers for the ImGui shell. Keeping every
		// one docked under the hidden inspector prevents stale registry settings
		// from exposing a second legacy MFC editor window.
		dockedTabs = W_CONSOLE | W_TEXTURE | W_MEDIA | W_ENTITY;
	}
