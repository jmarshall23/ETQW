	...
	if (patch) {
		...
		for ( i = 0; i < patch->width; i++ ) {
			for ( j = 0; j < patch->height; j++ ) {
				...
				patch->ctrl(i, j).SetST(
					f->brushprimit_texdef.coords[0][0] * x + f->brushprimit_texdef.coords[0][1] * y + f->brushprimit_texdef.coords[0][2],
	...
	do {
		...
		if (strcmp(token, "(")) {
			...
			if (newFormat) {
				...
				idWinding w;
				w.BaseForPlane( plane, MAX_WORLD_COORD );
