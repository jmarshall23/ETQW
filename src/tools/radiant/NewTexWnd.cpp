...
const idMaterial *CNewTexWnd::NextPos() {
	...
	declHolder.FindMaterial(mat->GetName());
...
const idMaterial *Texture_LoadLight(const char *name) {
	return declHolder.FindMaterial(name);
}
...
const idMaterial *Texture_ForName(const char *name) {
	...
	const idMaterial *mat = declHolder.FindMaterial(resolvedName);
	if ( !mat ) {
		mat = declHolder.FindMaterial("_default");
	} else {
		mat->SetMaterialFlag(MF_EDITOR_VISIBLE);
