	...
	for ( int i = 0; i < model->NumSurfaces(); i++ ) {
		...
		for ( int j = 0; j < tri->numIndexes; j += 3 ) {
			for ( int k = 0; k < 3; k++ ) {
				...
				const idVec2 st = tri->verts[index].GetST();
				qglTexCoord2f( st.x, st.y );
...
idWinding *Brush_MakeFaceWinding(brush_t *b, face_t *face, bool keepOnPlaneWinding) {
	...
	w = new idWinding( face->plane, MAX_WORLD_COORD );
	...
	for (clip = b->brush_faces; clip && w; clip = clip->next) {
		...
		if ( !w->ClipInPlace( plane, ON_EPSILON, keepOnPlaneWinding ) ) {
			delete w;
			return NULL;
		}
	}
	...
	if (model) {
		if ( model->IsDynamicModel() != DM_STATIC ) {
			if ( dynamic_cast<idRenderModelMD5 *>( model ) ) {
				...
				idDict animArgs = b->owner->epairs;
				animArgs.Set( "classname", classname );
				animArgs.Set( "anim", anim );
				animArgs.SetInt( "frame", frame );
				model = gameEdit->ANIM_CreateMeshForAnim( model, animArgs, false );
				if ( !model ) {
...
================
*/
static void DrawRadiantSphere( float radius, int slices, int stacks ) {
	if ( radius <= 0.0f ) {
		return;
	}
	for ( int stack = 0; stack < stacks; ++stack ) {
		const float latitude0 = -idMath::HALF_PI + idMath::PI * stack / stacks;
		const float latitude1 = -idMath::HALF_PI + idMath::PI * ( stack + 1 ) / stacks;
		const float z0 = radius * idMath::Sin( latitude0 );
		const float z1 = radius * idMath::Sin( latitude1 );
		const float ring0 = radius * idMath::Cos( latitude0 );
		const float ring1 = radius * idMath::Cos( latitude1 );
		qglBegin( GL_QUAD_STRIP );
		for ( int slice = 0; slice <= slices; ++slice ) {
			const float longitude = idMath::TWO_PI * slice / slices;
			const float x = idMath::Cos( longitude );
			const float y = idMath::Sin( longitude );
			qglVertex3f( ring0 * x, ring0 * y, z0 );
			qglVertex3f( ring1 * x, ring1 * y, z1 );
		}
		qglEnd();
	}
}
...
void DrawSpeaker(brush_t *b, bool bSelected, bool twoD) {
	...
	min *= RADIANT_METERS_TO_DOOM;
	max *= RADIANT_METERS_TO_DOOM;
	...
	if (twoD) {
		...
		qglPolygonMode (GL_FRONT_AND_BACK, GL_LINE);
		DrawRadiantSphere( min, 8, 8 );
		qglColor3f( 0.8f, 0.8f, 0.8f );
		DrawRadiantSphere( max, 8, 8 );
		qglEnable(GL_BLEND);
	...
	if ( model ) {
		...
		if ( model->IsDynamicModel() != DM_STATIC ) {
			if ( dynamic_cast<idRenderModelMD5 *>( model ) ) {
				...
				animArgs.Set( "anim", anim );
				animArgs.SetInt( "frame", frame );
				model2 = gameEdit->ANIM_CreateMeshForAnim( model, animArgs, false );
