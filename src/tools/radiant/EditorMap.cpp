...
void Map_Free(void) {
	...
	Texture_ClearInuse();
	Pointfile_Clear();
	strcpy(currentmap, "unnamed.world");
...
brush_t *BrushFromMapPatch(idMapPatch *mappatch, idVec3 origin) {
	...
	for (int i = 0; i < mappatch->GetWidth(); i++) {
		for (int j = 0; j < mappatch->GetHeight(); j++) {
			...
			pm->ctrl(i, j).SetST( (*mappatch)[j * mappatch->GetWidth() + i] );
		}
	}
...
brush_t *BrushFromMapBrush(idMapBrush *mapbrush, idVec3 origin) {
	...
	if (mapbrush) {
		...
		for (int i = 0; i < count; i++) {
			...
			idWinding w;
			w.BaseForPlane(f->plane, MAX_WORLD_COORD);
...
void Map_LoadFile(const char *filename) {
	...
	// global to unnamed.world, so retain an owned copy before freeing the old world.
	...
	RadiantImGuiTraceOpen( "Map_LoadFile entered", fileStr.c_str() );
	...
	// Radiant edits the source .world. Never let a compiled .entities sidecar
	// with the same basename hide its brushes from the editor.
	if(mapfile.Parse(filename, true, true, true, true)) {
		RadiantImGuiTraceOpen( "source parsed", va( "%d entities from %s", mapfile.GetNumEntities(), fileStr.c_str() ) );
		...
		RadiantImGuiTraceOpen( "source parse failed", fileStr.c_str() );
	}
	...
	RadiantImGuiTraceOpen( "Map_LoadFile complete", va( "title=%s entities=%d parsedBrushes=%d",
		currentmap, g_qeglobals.d_num_entities, g_qeglobals.d_parsed_brushes ) );
}
	...
	if ( b->pPatch ) {
		...
		for ( int i = 0; i < b->pPatch->width; i++ ) {
			for ( int j = 0; j < b->pPatch->height; j++ ) {
				...
				(*patch)[j*patch->GetWidth()+i].SetST( b->pPatch->ctrl(i, j) );
			}
		}
...
bool Map_SaveFile(const char *filename, bool use_region, bool autosave) {
	...
	brush_t		*b;
	idStr status;
	const bool sourceWasBlank = !idStr::Icmp( currentmap, "unnamed.world" );
	...
	if (filename == NULL || len == 0 || (filename && stricmp(filename, "unnamed.world") == 0)) {
		CFileDialog dlgSave(FALSE,"world",NULL,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,"ETQW World Files (*.world)|*.world||",AfxGetMainWnd());
	...
	idStr mapExt = (use_region) ? ".reg" : ".world";
