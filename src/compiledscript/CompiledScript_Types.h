...
public:
	...
	inline void ToWStringData( UINT_PTR& p ) const {
		p = reinterpret_cast< UINT_PTR >( value );
	}
...
public:
	...
	inline void ToIntegerData( UINT_PTR& p ) const {
		p = static_cast< UINT_PTR >( static_cast< int >( value ) );
	}
...
public:
	...
	inline			sdCompiledScriptType_Object( const sdCompiledScriptType_Object< OT >& _value ) { handle = _value.GetHandle(); }

					template< typename OT >
	inline sdCompiledScriptType_Object& operator=( const sdCompiledScriptType_Object< OT >& _value ) { handle = _value.GetHandle(); return *this; }
	...
	inline T*		GetObject( void ) { sdCompiledScript_ClassBase* temp = compilerInterface->GetObject( handle ); return temp ? temp->Cast< T >() : NULL; }
	...
	inline void ToObjectData( UINT_PTR& p ) const {
		p = reinterpret_cast< UINT_PTR >( compilerInterface->GetScriptObject( handle ) );
	}

	inline bool ToEntityData( UINT_PTR& p ) const {
		p = reinterpret_cast< UINT_PTR >( compilerInterface->GetEntity( handle ) );
		return p != NULL;
	}

	inline int		GetHandle( void ) const { return handle; }
