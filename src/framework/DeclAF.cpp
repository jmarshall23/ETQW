...
int idDeclAF::ContentsFromString( const char *str ) {
	...
	while( src.ReadToken( &token ) ) {
		...
		else if ( token.Icmp( "flyerhiveclip" ) == 0 ) {
			c |= CONTENTS_FLYERHIVECLIP;
		}
		else if ( token.Icmp( "vehicleclip" ) == 0 ) {
			c |= CONTENTS_VEHICLECLIP;
		}
		else if ( token.Icmp( "movableclip" ) == 0 ) {
			c |= CONTENTS_MOVEABLECLIP;
		}
		else if ( token == "," ) {
			continue;
		}
...
const char *idDeclAF::ContentsToString( const int contents, idStr &str ) {
	...
	if ( contents & CONTENTS_FLYERHIVECLIP ) {
		if ( str.Length() ) str += ", ";
		str += "flyerhiveclip";
	}
	...
	if ( contents & CONTENTS_MOVEABLECLIP ) {
		if ( str.Length() ) str += ", ";
		str += "movableclip";
	}
	if ( str[0] == '\0' ) {
