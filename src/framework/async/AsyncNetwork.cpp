...
idCVar				idAsyncNetwork::master0( "net_master0", "etqwmaster.idsoftware.com:27733", CVAR_SYSTEM | CVAR_ROM, "idnet master server address" );
...
idCVar				idAsyncNetwork::clientPunkbusterEnabled( "net_clientPunkbusterEnabled", "0", CVAR_BOOL | CVAR_ARCHIVE | CVAR_PROFILE, "is client side PunkBuster enabled?", 0, 1, idCmdSystem::ArgCompletion_Boolean );
idCVar				idAsyncNetwork::serverPunkbusterEnabled( "net_serverPunkbusterEnabled", "0", CVAR_BOOL | CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_RANKLOCKED, "is server side PunkBuster enabled?", 0, 1, idCmdSystem::ArgCompletion_Boolean );
...
void idAsyncNetwork::Init( void ) {
	...
	// These ETQW compatibility cvars were added after the Doom 3 networking
	// subset. Register them here as well as through the static-cvar list so
	// they are guaranteed to exist before the game DLL constructs its GUIs.
	cvarSystem->Register( &clientPunkbusterEnabled );
	cvarSystem->Register( &serverPunkbusterEnabled );
	...
	cmdSystem->AddCommand( "spawnServer", SpawnServer_f, CMD_FL_SYSTEM, "spawns a server", idCmdSystem::ArgCompletion_StartGame );
...
bool idAsyncNetwork::GetMasterAddress( int index, netadr_t &adr ) {
	...
	if ( !masters[ index ].resolved || masters[ index ].var->IsModified() ) {
		...
		if ( masters[ index ].address.port == 0 ) {
			masters[ index ].address.port = PORT_SERVER;
		}
		masters[ index ].resolved = true;
	}
...
==================
*/
void idAsyncNetwork::WriteUserCmdDelta( idBitMsg &msg, const usercmd_t &cmd, const usercmd_t *base, int timeOffset ) {
	const int gameTime = cmd.gameTime - timeOffset;

	if ( base ) {
		msg.WriteDeltaLongCounter( base->gameTime, gameTime );

		const bool movementChanged =
			base->buttons.btnValue != cmd.buttons.btnValue ||
			base->forwardmove != cmd.forwardmove ||
			base->rightmove != cmd.rightmove ||
			base->upmove != cmd.upmove;

		msg.WriteBits( movementChanged ? 1 : 0, 1 );
		if ( movementChanged ) {
			msg.WriteDelta( base->buttons.btnValue, cmd.buttons.btnValue, USERCMD_NUM_BUTTONS );
			msg.WriteDelta( base->forwardmove, cmd.forwardmove, -8 );
			msg.WriteDelta( base->rightmove, cmd.rightmove, -8 );
			msg.WriteDelta( base->upmove, cmd.upmove, -8 );
		}
		msg.WriteDelta( base->angles[0], cmd.angles[0], -16 );
		msg.WriteDelta( base->angles[1], cmd.angles[1], -16 );
		return;
	}

	msg.WriteBits( gameTime, 32 );
	msg.WriteBits( cmd.buttons.btnValue, USERCMD_NUM_BUTTONS );
	msg.WriteDelta( 0, cmd.forwardmove, -8 );
	msg.WriteDelta( 0, cmd.rightmove, -8 );
	msg.WriteDelta( 0, cmd.upmove, -8 );
	msg.WriteBits( cmd.angles[0], -16 );
	msg.WriteBits( cmd.angles[1], -16 );
}
...
void idAsyncNetwork::ReadUserCmdDelta( const idBitMsg &msg, usercmd_t &cmd, const usercmd_t *base ) {
	...
	if ( base ) {
		...
		if ( msg.ReadBits( 1 ) != 0 ) {
			cmd.buttons.btnValue = msg.ReadDelta( base->buttons.btnValue, USERCMD_NUM_BUTTONS );
			cmd.forwardmove = msg.ReadDelta( base->forwardmove, -8 );
			cmd.rightmove = msg.ReadDelta( base->rightmove, -8 );
			cmd.upmove = msg.ReadDelta( base->upmove, -8 );
		} else {
			cmd.buttons.btnValue = base->buttons.btnValue;
			cmd.forwardmove = base->forwardmove;
			cmd.rightmove = base->rightmove;
			cmd.upmove = base->upmove;
		}
		cmd.angles[0] = msg.ReadDelta( base->angles[0], -16 );
		cmd.angles[1] = msg.ReadDelta( base->angles[1], -16 );
		return;
	}

	cmd.gameTime = msg.ReadBits( 32 );
	cmd.buttons.btnValue = msg.ReadBits( USERCMD_NUM_BUTTONS );
	cmd.forwardmove = msg.ReadDelta( 0, -8 );
	cmd.rightmove = msg.ReadDelta( 0, -8 );
	cmd.upmove = msg.ReadDelta( 0, -8 );
	cmd.angles[0] = msg.ReadBits( -16 );
	cmd.angles[1] = msg.ReadBits( -16 );
}
...
bool idAsyncNetwork::DuplicateUsercmd( const usercmd_t &previousUserCmd, usercmd_t &currentUserCmd, int frame, int time ) {
	...
	if ( currentUserCmd.gameTime <= previousUserCmd.gameTime ) {
		...
		if ( currentUserCmd.duplicateCount > MAX_USERCMD_DUPLICATION ) {
			currentUserCmd.buttons.btn.attack = false;
...
bool idAsyncNetwork::UsercmdInputChanged( const usercmd_t &previousUserCmd, const usercmd_t &currentUserCmd ) {
	return	previousUserCmd.buttons.btnValue != currentUserCmd.buttons.btnValue ||
...
void idAsyncNetwork::ExecuteSessionCommand( const char *sessCmd ) {
	if ( sessCmd[ 0 ] ) {
		cmdSystem->BufferCommandText( CMD_EXEC_APPEND, sessCmd );
