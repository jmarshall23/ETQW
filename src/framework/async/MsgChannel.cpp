...
void idMsgChannel::WriteMessageData( idBitMsg &out, const idBitMsg &msg ) {
	...
	tmp.InitWrite( tmpBuf, sizeof( tmpBuf ) );
...
bool idMsgChannel::Process( const netadr_t from, int time, idBitMsg &msg, int &sequence ) {
	...
	if ( fragmented ) {
		...
		if ( fragLength < 0 || fragLength > msg.GetRemainingData() || fragmentLength + fragLength > sizeof( fragmentBuffer ) ) {
			if ( net_channelShowDrop.GetBool() || net_channelShowPackets.GetBool() ) {
		...
		memcpy( fragmentBuffer, msg.GetData() + msg.GetReadCount(), msg.GetRemainingData() );
		fragmentLength = msg.GetRemainingData();
		UpdatePacketLoss( time, 1, 0 );
	}

	fragMsg.InitRead( fragmentBuffer, fragmentLength );
