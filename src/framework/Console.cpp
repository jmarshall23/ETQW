...
public:
	...
	virtual	bool		ProcessEvent( const sdSysEvent *event, bool forceAccept );
...
private:
	bool				KeyDownEvent( const sdSysEvent *event );
...
void idConsoleLocal::LoadGraphics() {
	charSetShader = declHolder.FindMaterial( "textures/bigchars" );
	whiteShader = declHolder.FindMaterial( "_whiteVertexColor" );
	consoleShader = declHolder.FindMaterial( "console" );
}
...
====================
*/
bool idConsoleLocal::KeyDownEvent( const sdSysEvent *event ) {
	keyNum_e key = event->IsKeyEvent() ? event->GetKey() : K_INVALID;
	mouseButton_t mouseButton = event->IsMouseButtonEvent() ? event->GetMouseButton() : M_INVALID;
	
	// Execute F key bindings
	if ( key >= K_F1 && key <= K_F12 ) {
		idKey& standardKey = idKeyboard::GetStandardKey( key );
		sdBindContext* context = idKeyInput::GetContext( "default" );
		sdKeyCommand* command = context != NULL ? context->GetCommand( standardKey.GetId() ) : NULL;
		if ( command != NULL && command->GetType() == B_COMMAND ) {
	...
	if ( mouseButton == M_MWHEELUP ) {
		PageUp();
		return true;
	}

	if ( mouseButton == M_MWHEELDOWN ) {
		PageDown();
		return true;
	}
...
void idConsoleLocal::DrawInput() {
	...
	if ( consoleField.GetAutoCompleteLength() != 0 ) {
		...
		if ( autoCompleteLength > 0 ) {
			deviceContext->SetColor( .8f, .2f, .2f, .45f );

			deviceContext->DrawRect( 2 * SMALLCHAR_WIDTH + consoleField.GetAutoCompleteLength() * SMALLCHAR_WIDTH,
	...
	const float screenWidth = SCREEN_WIDTH / deviceContext->GetAspectRatioCorrection();
	consoleField.Draw( 2 * SMALLCHAR_WIDTH, y, idMath::FtoiFast( screenWidth ) - 3 * SMALLCHAR_WIDTH, true, charSetShader );
}
...
void idConsoleLocal::DrawNotify() {
	...
	deviceContext->SetColor( colorCyan );
}
...
void idConsoleLocal::DrawSolidConsole( float frac ) {
	...
	screenWidth = SCREEN_WIDTH / deviceContext->GetAspectRatioCorrection();
	...
	if ( y < 1.0f ) {
		...
		deviceContext->DrawRect( 0, 0, screenWidth, y, 0, 1.0f - displayFrac, 1, 1, consoleShader );
	}

	deviceContext->SetColor( colorCyan );
	deviceContext->DrawRect( 0, y, screenWidth, 2, 0, 0, 0, 0, whiteShader );
	...
	for ( x = 0; x < i; x++ ) {
		renderSystem->DrawChar( 7, 12, idMath::FtoiFast( screenWidth ) - ( i - x ) * 7 - 2,
			lines - 18, version[x], localConsole.charSetShader );
