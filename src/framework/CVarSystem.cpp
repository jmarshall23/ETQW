...
public:
	...
	void					Set( const char *newValue, bool force, bool clearLatched, bool fromServer );
	void					Reset( void );
...
private:
	...
	idStr					latchedString;			// applied by UpdateLatched
	idList< idCVarCallback* > callbacks;
	...
	virtual void			InternalServerSetString( const char *newValue, bool force, bool clearLatched );
...
============
*/
void idInternalCVar::Set( const char *newValue, bool force, bool clearLatched, bool fromServer ) {
	...
	if ( !force && ( flags & CVAR_LATCH ) ) {
		const char* currentValue = latchedString.Length() != 0 ? latchedString.c_str() : valueString.c_str();
		if ( idStr::Cmp( currentValue, newValue ) != 0 ) {
			common->Printf( "%s will be changed upon restarting.\n", nameString.c_str() );
			latchedString = newValue;
			SetModified();
...
/*
============
idInternalCVar callbacks and latched value
============
*/
void idInternalCVar::InternalRegisterCallback( idCVarCallback* callback ) {
	if ( callback != NULL && callbacks.FindIndex( callback ) == -1 ) {
		callbacks.Append( callback );
	}
}

void idInternalCVar::InternalUnRegisterCallback( idCVarCallback* callback ) {
	const int index = callbacks.FindIndex( callback );
	if ( index != -1 ) {
		callbacks.RemoveIndex( index );
	}
}

void idInternalCVar::InternalUpdateLatched() {
	if ( latchedString.Length() == 0 ) {
		return;
	}
	valueString = latchedString;
	latchedString.Clear();
	value = valueString.c_str();
	UpdateValue();
}

bool idInternalCVar::InternalIsDefaultValue() const {
	return resetString.Cmp( value ) == 0;
}
...
============
*/
void idInternalCVar::InternalSetString( const char *newValue, bool force, bool clearLatched ) {
	Set( newValue, force, clearLatched, false );
}
...
===============
*/
void idInternalCVar::InternalServerSetString( const char *newValue, bool force, bool clearLatched ) {
	Set( newValue, force, clearLatched, true );
}
...
============
*/
void idInternalCVar::InternalSetBool( const bool newValue, bool force, bool clearLatched ) {
	if ( newValue != ( integerValue != 0 ) ) {
		Set( idStr( newValue ), force, clearLatched, false );
	}
}
...
============
*/
void idInternalCVar::InternalSetInteger( const int newValue, bool force, bool clearLatched ) {
...
============
*/
void idInternalCVar::InternalSetFloat( const float newValue, bool force, bool clearLatched ) {
...
public:
	...
	virtual void			ResetFlaggedVariables( int flags, int ignoreFlags = 0 );
	...
	void					SetInternal( const char *name, const char *value, int flags, bool force, bool clearLatched );
...
============
*/
void idCVarSystemLocal::SetInternal( const char *name, const char *value, int flags, bool force, bool clearLatched ) {
	int hash;
	...
	if ( internal ) {
		internal->InternalSetString( value, force, clearLatched );
...
void idCVarSystemLocal::SetCVarString( const char *name, const char *value, int flags ) {
	SetInternal( name, value, flags, true, true );
}
...
void idCVarSystemLocal::SetCVarBool( const char *name, const bool value, int flags ) {
	SetInternal( name, idStr( value ), flags, true, true );
}
...
============
*/
void idCVarSystemLocal::ResetFlaggedVariables( int flags, int ignoreFlags ) {
	for( int i = 0; i < cvars.Num(); i++ ) {
		idInternalCVar *cvar = cvars[i];
		if ( ( cvar->GetFlags() & flags ) && !( cvar->GetFlags() & ignoreFlags ) ) {
			cvar->Set( NULL, true, true, false );
		}
	}
}
...
void idCVarSystemLocal::SetCVarsFromDict( const idDict &dict ) {
	...
	for( int i = 0; i < dict.GetNumKeyVals(); i++ ) {
		...
		if ( internal ) {
			internal->InternalServerSetString( kv->GetValue(), true, true );
		}
	}
}
...
void idCVarSystemLocal::Set_f( const idCmdArgs &args ) {
	...
	localCVarSystem.SetInternal( args.Argv( 1 ), str, 0, false, true );
}
