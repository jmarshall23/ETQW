...
class sdAddonInfo {
public:
	typedef idListGranularityOne< const idDict* > metaDataList_t;
	typedef sdHashMapGeneric< idStr, metaDataList_t > metaDataMap_t;

							sdAddonInfo() {}
							~sdAddonInfo();
	void					Swap( sdAddonInfo& rhs );

	idListGranularityOne< int >	depends;
	metaDataMap_t			metaData;
};
...
#if defined( _M_IX86 )
static_assert( sizeof( sdAddonInfo ) == 0x38, "sdAddonInfo must match the ETQW PDB layout" );
static_assert( sizeof( pack_t ) == 0x104C, "pack_t must match the ETQW PDB layout" );
#endif
...
class idFileSystemLocal : public idFileSystem, public sdThreadProcess {
public:
							idFileSystemLocal( void );

	virtual void			Init( bool quiet = false );
	virtual void			Restart( void );
	virtual void			Shutdown( bool reloading );
	virtual bool			IsInitialized( void ) const;
	virtual void			SetQuietMode( bool enable ) { quietMode = enable; }
	virtual bool			GetQuietMode( void ) const { return quietMode; }
	...
	virtual idFileList *	ListTools( void );
	...
	virtual const char *	GetBasePath( void ) const { return fs_basepath.GetString(); }
	virtual const char *	GetSavePath( void ) const { return fs_savepath.GetString(); }
	virtual const char *	GetUserPath( void ) const { return fs_userpath.GetString(); }
	virtual const char *	GetGamePath( void ) const { return gameFolder.c_str(); }
	...
	virtual fsPureReply_t	SetPureServerChecksums( const int pureChecksums[ MAX_PURE_PAKS ], int gamePakChecksum, int missingChecksums[ MAX_PURE_PAKS ], int *missingGamePakChecksum, const char* serverVersion = NULL );
	...
	virtual bool			FileExistsExplicit( const char* OSPath );
	virtual unsigned int	GetTimestamp( const char* relativePath );
	virtual int				ReadFile( const char *relativePath, void **buffer, unsigned int *timestamp = NULL, bool markPaksReferenced = true );
	...
	virtual idFile *		OpenFileRead( const char *relativePath, bool allowCopyFiles = true, const char* gamedir = NULL, bool markPaksReferenced = true );
	...
	virtual void			RemoveExplicitFile( const char* OSPath );
	virtual bool			RemoveExplicitDir( const char* OSPath, bool nonEmpty, bool recursive );
	virtual void			CloseFile( idFile *f );
	virtual void			BackgroundDownload( backgroundDownload_t *bgl );
	virtual bool			CancelBackgroundDownload( backgroundDownload_t* bgl );
	...
	virtual idFile_Memory*	OpenMemoryFile( const char* name );
	virtual idFile_Buffered* OpenBufferedFile( idFile* file );
	...
	virtual int				AddPackFile( const char *path );
	virtual findFile_t		FindFile( const char *path );
	virtual bool			FilenameCompare( const char *s1, const char *s2 ) const;
	virtual unsigned long	FileChecksum( idFile* file, unsigned char* md5digest = NULL );
	virtual unsigned long	FileChecksum( FILE* file, unsigned char* md5digest = NULL );
	virtual void			BeginLevelLoadStatistics();
	virtual void			AddLevelLoadStatistic( const char* extension, int numBytes );
	virtual void			EndLevelLoadStatistics();
	virtual void			ReportLevelLoadStatistics();
	virtual bool			IsAddonPackReferenced( const char* pak );
	virtual void			ReferenceAddonPack( const char* pak );
	virtual void			ReadTGA( const char* name, byte** pic, int* width, int* height, unsigned* timestamp = NULL, bool markPaksReferenced = true );
...
private:
	void					StartBackgroundDownloadThread();
	void					StopBackgroundDownloadThread();
	unsigned int			BackgroundDownloadThread( void* parms );

	sdThread*				bglThread;
	sdSignal				bglSignal;
	sdSignal				bglOpSignal;
	sdLock					bglLock;
	bool					quietMode;
	...
	static idCVar			fs_userpath;
	static idCVar			fs_cdpath;
	static idCVar			fs_devpath;
	static idCVar			fs_toolsPath;
...
private:
	...
	pack_t *				LoadPackFile( const char *zipfile );
	...
	static bool				AddMetaData( const idList< const idDict* >& list, idList< metaDataContext_t >& metaData, sdHashMapGeneric< idStr, int >& metaDataIndices, const char* pak, bool addon );
	sdAddonInfo *			ParseAddonDef( const char *buf, const int len, addonType_t addonType );
	void					FollowAddonDependencies( pack_t *pak );
	bool					ValidateDownloadPak( pack_t* pak, char path[ MAX_STRING_CHARS ], bool& isBinaryReturn );
	void					ReferenceAddonPack_r( pack_t* pack );
...
#if defined( _M_IX86 )
static_assert( sizeof( idFileSystemLocal ) == 0x7C0, "idFileSystemLocal must match the ETQW PDB layout" );
#endif
...
idCVar	idFileSystemLocal::fs_userpath( "fs_userpath", "", CVAR_SYSTEM | CVAR_INIT, "" );
...
idCVar	idFileSystemLocal::fs_toolsPath( "fs_toolsPath", "", CVAR_SYSTEM | CVAR_INIT, "semicolon separated tool search paths" );
...
/*
================
sdAddonInfo::~sdAddonInfo
================
*/
sdAddonInfo::~sdAddonInfo() {
	for ( metaDataMap_t::Iterator group = metaData.Begin(); group != metaData.End(); ++group ) {
		for ( int i = 0; i < group->second.Num(); i++ ) {
			delete group->second[ i ];
		}
	}
}
...
================
*/
void sdAddonInfo::Swap( sdAddonInfo& rhs ) {
	depends.Swap( rhs.depends );
...
idFileSystemLocal::idFileSystemLocal( void ) :
	...
	pureRemapGamePakChecksum = 0;
	addonPaks = NULL;
	defaultBackgroundDownload.next = NULL;
	defaultBackgroundDownload.opcode = DLTYPE_FILE;
	defaultBackgroundDownload.fh = NULL;
	defaultBackgroundDownload.file.offset = 0;
	...
	defaultBackgroundDownload.url.curlstatus = 0;
...
/*
=================
idFileSystemLocal::OpenMemoryFile
=================
*/
idFile_Memory* idFileSystemLocal::OpenMemoryFile( const char* name ) {
	return new idFile_Memory( name );
}

/*
=================
idFileSystemLocal::OpenBufferedFile
=================
*/
idFile_Buffered* idFileSystemLocal::OpenBufferedFile( idFile* file ) {
	return new idFile_Buffered( file, 2 * 1024 * 1024 );
}
	...
	for ( search = searchPaths; search; search = search->next ) {
		...
		if ( search->pack && search->pack->hashTable[hash] ) {
			...
			if ( serverPaks.Num() ) {
				...
				if ( search->pack->pureStatus != PURE_NEVER && serverPaks.FindIndex( search->pack ) < 0 ) {
...
bool idFileSystemLocal::FileExists( const char* relativePath ) {
	return ReadFile( relativePath, NULL, NULL, true ) != -1;
}

/*
============
idFileSystemLocal::FileExistsExplicit
============
*/
bool idFileSystemLocal::FileExistsExplicit( const char* OSPath ) {
	FILE* file = OpenOSFile( OSPath, "rb" );
	if ( !file ) {
		return false;
	}
	fclose( file );
...
/*
============
idFileSystemLocal::GetTimestamp
============
*/
unsigned int idFileSystemLocal::GetTimestamp( const char* relativePath ) {
	...
	if ( !relativePath || !relativePath[ 0 ] ) {
		common->FatalError( "idFileSystemLocal::GetTimestamp with empty name\n" );
	}

	idFile* file = OpenFileRead( relativePath, false, NULL, true );
	if ( !file ) {
		return FILE_NOT_FOUND_TIMESTAMP;
	}
	const unsigned int timestamp = file->Timestamp();
	CloseFile( file );
	return timestamp;
}
...
============
*/
int idFileSystemLocal::ReadFile( const char *relativePath, void **buffer, unsigned int *timestamp, bool markPaksReferenced ) {
	idFile *	f;
	byte *		buf;
	int			len;
	bool		isConfig;
	...
	f = OpenFileRead( relativePath, ( buffer != NULL ), NULL, markPaksReferenced );
	if ( f == NULL ) {
		if ( buffer ) {
			*buffer = NULL;
		}
...
=================
*/
sdAddonInfo *idFileSystemLocal::ParseAddonDef( const char *buf, const int len, addonType_t addonType ) {
	idLexer src;
	idToken token;
	sdAddonInfo info;
	...
	if ( addonType == ADDON_CONFIG_TYPE ) {
	...
	while ( src.ReadToken( &token ) ) {
		const idStr type = token;
		if ( !src.ReadToken( &token ) || token.IsEmpty() || !token.Icmp( "{" ) ) {
			src.Warning( "Expected metaData name" );
			return NULL;
		}
		...
		if ( !dict->Parse( src ) ) {
			src.Warning( "Failed to parse metaData info" );
			delete dict;
			return NULL;
		}
		info.metaData[ type ].Append( dict );
	}

	sdAddonInfo* result = new sdAddonInfo;
	result->Swap( info );
	return result;
}
...
/*
=================
idFileSystemLocal::LoadPackFile
=================
*/
pack_t *idFileSystemLocal::LoadPackFile( const char *zipfile ) {
	fileInPack_t *	buildBuffer;
	pack_t *		pack;
	unzFile			uf;
	int				err;
	...
	for ( pakFile = pack->hashTable[confHash]; pakFile; pakFile = pakFile->next ) {
		if ( !FilenameCompare( pakFile->name, ADDON_CONFIG ) ) {			
			...
			if ( file && file->Length() ) {
				...
				buf[ file->Length() ] = '\0';
				pack->addOnInfo = ParseAddonDef( buf, file->Length(), ADDON_CONFIG_TYPE );
				delete[] buf;
			}
			if ( file ) {
				CloseFile( file );
	...
	// Normal paks may publish map and campaign metadata without becoming
	// restart-controlled addon paks.
	if ( !pack->addon ) {
		confHash = HashFileName( "pakmeta.conf" );
		for ( pakFile = pack->hashTable[ confHash ]; pakFile; pakFile = pakFile->next ) {
			if ( FilenameCompare( pakFile->name, "pakmeta.conf" ) ) {
				continue;
			}
			idFile_InZip* file = ReadFileFromZip( pack, pakFile, "pakmeta.conf" );
			if ( file && file->Length() ) {
				const int fileLength = file->Length();
				char* buf = new char[ fileLength + 1 ];
				file->Read( buf, fileLength );
				buf[ fileLength ] = '\0';
				pack->addOnInfo = ParseAddonDef( buf, fileLength, PAKMETA_CONFIG_TYPE );
				delete[] buf;
			}
			if ( file ) {
				CloseFile( file );
...
===============
*/
int idFileSystemLocal::AddPackFile( const char *path ) {
	idStr			fullpath = fs_savepath.GetString();
	pack_t			*pak;
	searchpath_t	*search, *last;

	fullpath.AppendPath( path );
	pak = LoadPackFile( fullpath );
	if ( !pak ) {
		common->Warning( "AddPackFile %s failed\n", path );
		return 0;
	}
...
int idFileSystemLocal::AddUnique( const char *name, idStrList &list, idHashIndex &hashIndex ) const {
	...
	for ( i = hashIndex.GetFirst( hashKey ); i >= 0; i = hashIndex.GetNext( i ) ) {
...
idModList *idFileSystemLocal::ListMods( void ) {
	...
	for ( isearch = 0; isearch < 4; isearch++ ) {
		...
		for( i = 0; i < dirs.Num(); i++ ) {
			...
			if ( pk4s.Num() ) {
				if ( list->mods.FindIndex( dirs[ i ] ) < 0 ) {
...
/*
================
idFileSystemLocal::AddMetaData
================
*/
bool idFileSystemLocal::AddMetaData( const idList< const idDict* >& list, idList< metaDataContext_t >& metaData, sdHashMapGeneric< idStr, int >& metaDataIndices, const char* pak, bool addon ) {
	bool foundMetaData = false;

	for ( int i = 0; i < list.Num(); i++ ) {
		const idDict* source = list[ i ];
		const char* name = source->GetString( "metadata_name" );
		if ( !name[ 0 ] ) {
			continue;
		}

		metaDataContext_t newContext;
		newContext.meta = new idDict( *source );
		newContext.addon = addon;
		newContext.pak = pak;

		sdHashMapGeneric< idStr, int >::Iterator existing = metaDataIndices.Find( name );
		if ( existing == metaDataIndices.End() ) {
			const int index = metaData.Append( newContext );
			metaDataIndices.Set( name, index );
		} else {
			metaDataContext_t& oldContext = metaData[ existing->second ];
			delete oldContext.meta;
...
/*
================
idFileSystemLocal::ListAddonMetaData
================
*/
sdAddonMetaDataList* idFileSystemLocal::ListAddonMetaData( const char* metaDataTag ) {
	sdAddonMetaDataList* result = new sdAddonMetaDataList;
	sdHashMapGeneric< idStr, int > metaDataIndices;
	bool sawPack = false;

	for ( searchpath_t* search = addonPaks; search; search = search->next ) {
		if ( !search->pack ) {
			continue;
		}
		sawPack = true;
		if ( !search->pack->addon || !search->pack->addOnInfo ) {
			continue;
		}
		sdAddonInfo::metaDataMap_t::Iterator group = search->pack->addOnInfo->metaData.Find( metaDataTag );
		if ( group != search->pack->addOnInfo->metaData.End() ) {
			AddMetaData( group->second, result->meta, metaDataIndices, search->pack->pakFilename, true );
		}
	}
	...
	for ( searchpath_t* search = searchPaths; search; search = search->next ) {
		...
		sawPack = true;
		if ( !search->pack->addOnInfo ) {
			continue;
		}
		sdAddonInfo::metaDataMap_t::Iterator group = search->pack->addOnInfo->metaData.Find( metaDataTag );
		if ( group != search->pack->addOnInfo->metaData.End() ) {
			AddMetaData( group->second, result->meta, metaDataIndices, search->pack->pakFilename, false );
		}
	}

	// Development layouts without loaded paks can provide the same metadata
	// directly in the current game directory.
	if ( !sawPack ) {
		idFile* file = OpenFileRead( "pakmeta.conf", true, NULL, true );
		if ( file ) {
			...
			if ( length > 0 ) {
				...
				buffer[ length ] = '\0';
				sdAddonInfo* info = ParseAddonDef( buffer, length, PAKMETA_CONFIG_TYPE );
				if ( info ) {
					sdAddonInfo::metaDataMap_t::Iterator group = info->metaData.Find( metaDataTag );
					if ( group != info->metaData.End() ) {
						AddMetaData( group->second, result->meta, metaDataIndices, "", false );
					}
					delete info;
				}
				delete[] buffer;
...
/*
================
idFileSystemLocal::FreeAddonMetaDataList
================
*/
...
/*
================
idFileSystemLocal::ListTools
================
*/
idFileList* idFileSystemLocal::ListTools( void ) {
	idHashIndex hashIndex( 4096, 4096 );
	idStrList extensionList;
	idCmdArgs paths( fs_toolsPath.GetString(), false );
	idFileList* result = new idFileList;

	GetExtensionList( ".dll", extensionList );

	if ( serverPaks.Num() == 0 ) {
		idStr executablePath = Sys_EXEPath();
		executablePath.StripFilename();
		idStrList files;
		for ( int i = 0; i < extensionList.Num(); i++ ) {
			ListOSFiles( executablePath, extensionList[ i ], files );
			for ( int j = 0; j < files.Num(); j++ ) {
				AddUnique( files[ j ], result->list, hashIndex );
			}
			files.Clear();
		}
	}

	for ( int i = 0; i < paths.Argc(); i++ ) {
		const char* toolPath = paths.Argv( i );
		if ( !idStr::Icmp( toolPath, "." ) || !idStr::Icmp( toolPath, ";" ) ) {
			continue;
		}
		GetFileList( toolPath, extensionList, result->list, hashIndex, true );
	}
	...
	for ( sp = fileSystemLocal.searchPaths; sp; sp = sp->next ) {
		if ( sp->pack ) {
			...
			if ( fileSystemLocal.serverPaks.Num() ) {
				if ( fileSystemLocal.serverPaks.FindIndex( sp->pack ) >= 0 ) {
...
void idFileSystemLocal::FollowAddonDependencies( pack_t *pak ) {
	assert( pak );
	if ( !pak->addOnInfo || !pak->addOnInfo->depends.Num() ) {
		return;
	}
	int i, num = pak->addOnInfo->depends.Num();
	for ( i = 0; i < num; i++ ) {
		pack_t *deppak = GetPackForChecksum( pak->addOnInfo->depends[ i ], true );
		if ( deppak ) {
			...
			common->Printf( "Addon pk4 %s 0x%x depends on unknown pak 0x%x\n",
							pak->pakFilename.c_str(), pak->checksum, pak->addOnInfo->depends[ i ] );
		}
	}
}
...
/*
===============
idFileSystemLocal::ValidateDownloadPak
===============
*/
bool idFileSystemLocal::ValidateDownloadPak( pack_t* pak, char path[ MAX_STRING_CHARS ], bool& isBinaryReturn ) {
	(void)path;
	if ( !pak ) {
		return false;
	}

	idStr name = pak->pakFilename;
	name.StripPath();
	...
	isBinaryReturn = pak->binary == BINARY_YES;
	return true;
}
...
/*
===============
idFileSystemLocal::ValidateDownloadPakForRelativePath
===============
*/
int idFileSystemLocal::ValidateDownloadPakForRelativePath( const char* relativePath, char path[ MAX_STRING_CHARS ], bool& isBinaryReturn ) {
	pack_t* found = NULL;
	const int relativeLength = idStr::Length( relativePath );

	for ( searchpath_t* search = searchPaths; search; search = search->next ) {
		pack_t* pak = search->pack;
		if ( !pak || pak->pakFilename.Length() <= relativeLength ) {
			continue;
		}
		const int prefixLength = pak->pakFilename.Length() - relativeLength;
		const char separator = pak->pakFilename[ prefixLength - 1 ];
		if ( ( separator == '\\' || separator == '/' ) &&
			 !idStr::Cmp( pak->pakFilename.c_str() + prefixLength, relativePath ) ) {
			found = pak;
		}
	}

	if ( !ValidateDownloadPak( found, path, isBinaryReturn ) ) {
		return 0;
	}
	idStr::Copynz( path, found->pakFilename, MAX_STRING_CHARS );
	return found->length;
}
...
=====================
*/
fsPureReply_t idFileSystemLocal::SetPureServerChecksums( const int pureChecksums[ MAX_PURE_PAKS ], int _gamePakChecksum, int missingChecksums[ MAX_PURE_PAKS ], int *missingGamePakChecksum, const char* serverVersion ) {
	pack_t			*pack;
	int				i, j, imissing;
	bool			success = true;
			...
			if ( pack ) {
				if ( canPrepend ) {
					...
					if ( fs_debug.GetBool() ) {
						...
						if ( serverPaks.FindIndex( pack ) >= 0 ) {
...
================
*/
void idFileSystemLocal::Init( bool quiet ) {
	quietMode = quiet;
	...
	common->StartupVariable( "fs_userpath" );
	common->StartupVariable( "fs_cdpath" );
	...
	common->StartupVariable( "fs_searchAddons" );
	...
	if ( fs_userpath.GetString()[0] == '\0' ) {
		fs_userpath.SetString( fs_savepath.GetString() );
	}
	...
	for ( loop = searchPaths; loop; loop == searchPaths ? loop = addonPaks : loop = NULL ) {
		for ( sp = loop; sp; sp = next ) {
			...
			if ( sp->pack ) {
				...
				if ( sp->pack->addOnInfo ) {
					delete sp->pack->addOnInfo;
				}
				delete sp->pack;
			}
			if ( sp->dir ) {
...
===========
*/
idFile *idFileSystemLocal::OpenFileRead( const char *relativePath, bool allowCopyFiles, const char* gamedir, bool markPaksReferenced ) {
	int searchFlags = FSFLAG_SEARCH_DIRS | FSFLAG_SEARCH_PAKS;
	if ( !markPaksReferenced ) {
		searchFlags |= FSFLAG_PURE_NOREF;
	}
	return OpenFileReadFlags( relativePath, searchFlags, NULL, allowCopyFiles, gamedir );
}
...
===========
*/
void idFileSystemLocal::RemoveExplicitFile( const char* OSPath ) {
	const int savePathLength = idStr::Length( fs_savepath.GetString() );
	const int userPathLength = idStr::Length( fs_userpath.GetString() );
	if ( idStr::IcmpnPath( OSPath, fs_savepath.GetString(), savePathLength ) &&
		 idStr::IcmpnPath( OSPath, fs_userpath.GetString(), userPathLength ) ) {
		return;
	}

	const int stat = sys->File_Stat( OSPath );
	if ( stat == 0 ) {
		remove( OSPath );
		ClearDirCache();
	} else if ( stat == 1 ) {
		RemoveExplicitDir( OSPath, true, true );
		ClearDirCache();
	}
}
...
/*
===========
idFileSystemLocal::RemoveExplicitDir
===========
*/
bool idFileSystemLocal::RemoveExplicitDir( const char* OSPath, bool nonEmpty, bool recursive ) {
	const int savePathLength = idStr::Length( fs_savepath.GetString() );
	const int userPathLength = idStr::Length( fs_userpath.GetString() );
	if ( idStr::IcmpnPath( OSPath, fs_savepath.GetString(), savePathLength ) &&
		 idStr::IcmpnPath( OSPath, fs_userpath.GetString(), userPathLength ) ) {
	...
	idStrList entries;
	if ( recursive ) {
		Sys_ListFiles( OSPath, "/", entries );
		entries.Remove( "." );
		entries.Remove( ".." );
		for ( int i = 0; i < entries.Num(); i++ ) {
			const idStr directory = va( "%s/%s", OSPath, entries[ i ].c_str() );
			if ( !RemoveExplicitDir( directory, nonEmpty, recursive ) ) {
				return false;
			}
		}
		entries.Clear();
	...
	if ( nonEmpty ) {
		Sys_ListFiles( OSPath, NULL, entries );
		for ( int i = 0; i < entries.Num(); i++ ) {
			const idStr filename = va( "%s/%s", OSPath, entries[ i ].c_str() );
...
size_t idFileSystemLocal::CurlWriteFunction( void *ptr, size_t size, size_t nmemb, void *stream ) {
	...
	return fwrite( ptr, 1, size * nmemb, static_cast<idFile_Permanent*>(bgl->fh)->GetFilePtr() );
}
...
===================
*/
unsigned int idFileSystemLocal::BackgroundDownloadThread( void* parms ) {
	while ( !Terminating() ) {
		bglLock.Acquire();
		backgroundDownload_t* bgl = backgroundDownloads;
		if ( bgl != NULL ) {
			backgroundDownloads = bgl->next;
			bgl->next = NULL;
		}
		bglLock.Release();

		if ( bgl == NULL ) {
			threadWaitingForObject = true;
			bglSignal.Wait();
			threadWaitingForObject = false;
			continue;
		}
		...
		if ( bgl->opcode == DLTYPE_FILE ) {
			bgl->fh->Seek( bgl->file.offset, FS_SEEK_SET );
			bgl->fh->Read( bgl->file.buffer, bgl->file.length );
			bgl->completed = true;
		} else {
#if ID_ENABLE_CURL
			// DLTYPE_URL
			...
			CURLcode ret;
			if ( !session ) {
				bgl->url.curlstatus = CURLE_FAILED_INIT;
...
unsigned int idFileSystemLocal::Run( void* parm ) {
	return BackgroundDownloadThread( parm );
}
...
void idFileSystemLocal::StartBackgroundDownloadThread() {
	...
	bglThread = new sdThread( this, THREAD_NORMAL );
	bglThread->SetName( "backgroundDownload" );
	if ( !bglThread->Start() ) {
...
void idFileSystemLocal::BackgroundDownload( backgroundDownload_t *bgl ) {
	...
	if ( bgl->opcode == DLTYPE_FILE && dynamic_cast< idFile_Permanent* >( bgl->fh ) == NULL ) {
		bgl->fh->Seek( bgl->file.offset, FS_SEEK_SET );
		bgl->fh->Read( bgl->file.buffer, bgl->file.length );
		bgl->completed = true;
		return;
...
bool idFileSystemLocal::CancelBackgroundDownload( backgroundDownload_t* bgl ) {
	if ( bgl == NULL || bgl->completed ) {
		return false;
	}
...
/*
=================
idFileSystemLocal::FileChecksum
=================
*/
unsigned long idFileSystemLocal::FileChecksum( idFile* file, unsigned char* md5digest ) {
	const int blockSize = 8 * 1024 * 1024;
	byte* buffer = static_cast< byte* >( Mem_Alloc( blockSize ) );
	unsigned char localDigest[ 16 ];
	md5Context_t context;

	file->Seek( 0, FS_SEEK_END );
	int remaining = file->Tell();
	file->Seek( 0, FS_SEEK_SET );

	MD5_StartChecksum( context );
	while ( remaining > 0 ) {
		const int count = remaining < blockSize ? remaining : blockSize;
		if ( file->Read( buffer, count ) != count ) {
			Mem_Free( buffer );
			common->FatalError( "Short read in idFileSystemLocal::GetFileChecksum()\n" );
		}
		MD5_UpdateChecksum( context, buffer, count );
		remaining -= count;
	}
	Mem_Free( buffer );

	const unsigned long checksum = MD5_FinishChecksum( context, localDigest );
	if ( md5digest ) {
		memcpy( md5digest, localDigest, sizeof( localDigest ) );
	}
	return checksum;
}
...
=================
*/
unsigned long idFileSystemLocal::FileChecksum( FILE* file, unsigned char* md5digest ) {
	const int blockSize = 8 * 1024 * 1024;
	const long originalPosition = ftell( file );
	...
	while ( remaining > 0 ) {
		const int count = remaining < blockSize ? remaining : blockSize;
		if ( fread( buffer, 1, count, file ) != static_cast< size_t >( count ) ) {
			Mem_Free( buffer );
			common->FatalError( "Short read in idFileSystemLocal::FileChecksum()\n" );
		}
		MD5_UpdateChecksum( context, buffer, count );
		remaining -= count;
	}
...
/*
=================
idFileSystemLocal level load statistics

The shipping Microsoft build compiles these four hooks to no-ops.
=================
*/
void idFileSystemLocal::BeginLevelLoadStatistics() {
}

void idFileSystemLocal::AddLevelLoadStatistic( const char* extension, int numBytes ) {
	(void)extension;
	(void)numBytes;
}

void idFileSystemLocal::EndLevelLoadStatistics() {
}

void idFileSystemLocal::ReportLevelLoadStatistics() {
}

/*
=================
idFileSystemLocal::IsAddonPackReferenced
=================
*/
bool idFileSystemLocal::IsAddonPackReferenced( const char* pak ) {
	for ( searchpath_t* search = addonPaks; search; search = search->next ) {
		if ( !search->pack->addon || !search->pack->addOnInfo ||
			 idStr::IcmpPath( search->pack->pakFilename, pak ) ) {
			continue;
		}
		return addonChecksums.FindIndex( search->pack->checksum ) != -1;
	}
	return false;
}
...
/*
=================
idFileSystemLocal::ReferenceAddonPack_r
=================
*/
void idFileSystemLocal::ReferenceAddonPack_r( pack_t* pack ) {
	pack->referenced = true;
	addonChecksums.Append( pack->checksum );

	for ( int i = 0; i < pack->addOnInfo->depends.Num(); i++ ) {
		const int checksum = pack->addOnInfo->depends[ i ];
		if ( addonChecksums.FindIndex( checksum ) != -1 ) {
			continue;
		}
		for ( searchpath_t* search = addonPaks; search; search = search->next ) {
			if ( search->pack->checksum == checksum ) {
				ReferenceAddonPack_r( search->pack );
			}
		}
	}
}
...
/*
=================
idFileSystemLocal::ReferenceAddonPack
=================
*/
void idFileSystemLocal::ReferenceAddonPack( const char* pak ) {
	for ( searchpath_t* search = addonPaks; search; search = search->next ) {
		if ( search->pack->addOnInfo && !idStr::IcmpPath( search->pack->pakFilename, pak ) ) {
			ReferenceAddonPack_r( search->pack );
		}
	}
}
...
=================
*/
void idFileSystemLocal::FindDLL( const char *name, char _dllPath[ MAX_OSPATH ], bool updateChecksum, bool pureCheck ) {
	idFile			*dllFile = NULL;
	char			dllName[MAX_OSPATH];
	idStr			dllPath;
	int				dllHash;
...
#if 0 // Doom 3 expansion probing is not part of the ETQW filesystem.
/*
===============
...
findFile_t idFileSystemLocal::FindFile( const char *path ) {
	...
	if ( pak->addon && addonChecksums.FindIndex( pak->checksum ) < 0 ) {
...
#if 0 // ETQW map metadata is exposed through addon metadata, not Doom mapDef decls.
/*
===============
