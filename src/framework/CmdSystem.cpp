...
typedef struct aliasDef_s {
	struct aliasDef_s *		next;
	char *					alias;
	char *					cmd;
} aliasDef_t;
...
public:
	...
	virtual void			AddAlias( const char *alias, const char *command );
	virtual void			RemoveAlias( const char *alias );
	virtual const char *	FindAliasForValue( const char *value ) const;

	virtual void			HandleFrameCommands( frameCommandCallback_t callback );
	...
	virtual void			ExecuteCommandBuffer( bool allowWait = true );
	...
	virtual void			ArgCompletion_DeclName( const idCmdArgs &args, void(*callback)( const char *s ), const char *typeName );
...
private:
	...
	int						wait;
	int						textLengths[2];
	char					textBuffers[2][MAX_CMD_BUFFER];
	int *					textLength;
	char *					textBuf;

	idStr					completionString;
	idStrList				completionParms;
	idStrList				frameCommandList;
...
private:	
	aliasDef_t *			FindAlias( const char *alias ) const;
...
void idCmdSystemLocal::Init( void ) {
	...
	textLengths[0] = 0;
	textLengths[1] = 0;
	textLength = &textLengths[0];
	textBuf = textBuffers[0];
}
...
void idCmdSystemLocal::Shutdown( void ) {
	...
	for ( alias = aliases; alias; alias = aliases ) {
		aliases = alias->next;
		Mem_Free( alias->alias );
		Mem_Free( alias->cmd );
		delete alias;
	}
...
/*
============
idCmdSystemLocal::FindAlias
============
*/
aliasDef_t *idCmdSystemLocal::FindAlias( const char *alias ) const {
	aliasDef_t *aliasDef;

	for ( aliasDef = aliases; aliasDef; aliasDef = aliasDef->next ) {
		if ( idStr::Icmp( alias, aliasDef->alias ) == 0 ) {
			return aliasDef;
		}
	}
	return NULL;
...
/*
============
idCmdSystemLocal::AddAlias
============
*/
void idCmdSystemLocal::AddAlias( const char *alias, const char *command ) {
	aliasDef_t *aliasDef;

	if ( FindAlias( alias ) != NULL ) {
		common->Printf( "idCmdSystemLocal::AddAlias: %s already defined\n", alias );
		return;
	}

	aliasDef = new aliasDef_t;
	aliasDef->alias = Mem_CopyString( alias );
	aliasDef->cmd = Mem_CopyString( command );
	aliasDef->next = aliases;
	aliases = aliasDef;
}

/*
============
idCmdSystemLocal::RemoveAlias
============
*/
void idCmdSystemLocal::RemoveAlias( const char *alias ) {
	aliasDef_t *aliasDef;
	aliasDef_t **previous;

	for ( previous = &aliases, aliasDef = *previous; aliasDef; aliasDef = *previous ) {
		if ( idStr::Icmp( alias, aliasDef->alias ) == 0 ) {
			*previous = aliasDef->next;
			Mem_Free( aliasDef->alias );
			Mem_Free( aliasDef->cmd );
			delete aliasDef;
			return;
		}
		previous = &aliasDef->next;
...
/*
============
idCmdSystemLocal::FindAliasForValue

Compare lexer tokens so harmless differences in whitespace and quoting do not
prevent a saved binding from resolving back to its alias.
============
*/
const char *idCmdSystemLocal::FindAliasForValue( const char *value ) const {
	for ( aliasDef_t *aliasDef = aliases; aliasDef; aliasDef = aliasDef->next ) {
		idLexer valueLexer( value, idStr::Length( value ), "FindAliasForValue" );
		idLexer aliasLexer( aliasDef->cmd, idStr::Length( aliasDef->cmd ), "FindAliasForValue" );
		idToken valueToken;
		idToken aliasToken;

		for ( ;; ) {
			const bool hasValueToken = valueLexer.ReadToken( &valueToken ) != 0;
			const bool hasAliasToken = aliasLexer.ReadToken( &aliasToken ) != 0;

			if ( hasValueToken != hasAliasToken ) {
				break;
			}
			if ( !hasValueToken ) {
				return aliasDef->alias;
			}
			if ( valueToken.Icmp( aliasToken ) != 0 ) {
				break;
			}
		}
	}
...
/*
============
idCmdSystemLocal::PushFrameCommand
============
*/
void idCmdSystemLocal::PushFrameCommand( const char *command ) {
	frameCommandList.Append( command );
}

/*
============
idCmdSystemLocal::HandleFrameCommands
============
*/
void idCmdSystemLocal::HandleFrameCommands( frameCommandCallback_t callback ) {
	if ( frameCommandList.Num() == 0 || !callback.IsValid() ) {
		return;
	}

	// Commands queued by a callback belong to the next frame.
	idStrList frameCommandCopy = frameCommandList;
	frameCommandList.Clear();

	for ( int i = 0; i < frameCommandCopy.Num(); i++ ) {
		idCmdArgs args;
		args.TokenizeString( frameCommandCopy[i], false );
		callback( args );
	}
}
...
void idCmdSystemLocal::ExecuteTokenizedString( const idCmdArgs &args ) {	
	...
	const idCmdArgs *commandArgs = &args;
	...
	aliasDef_t *alias = FindAlias( args.Argv( 0 ) );
	if ( alias != NULL ) {
		idStr expanded = alias->cmd;
		const char *trailingArgs = args.Args( 1, -1 );
		if ( trailingArgs[0] != '\0' ) {
			expanded += " ";
			expanded += trailingArgs;
		}
		expandedArgs.TokenizeString( expanded, false );
		commandArgs = &expandedArgs;
	} else {
		expandedArgs = args;
		commandArgs = &expandedArgs;
	}
	...
	for ( prev = &commands; *prev; prev = &cmd->next ) {
		cmd = *prev;
		if ( idStr::Icmp( commandArgs->Argv( 0 ), cmd->name ) == 0 ) {
	...
	// check cvars
	if ( cvarSystem->Command( *commandArgs ) ) {
		return;
	}
...
void idCmdSystemLocal::ExecuteCommandText( const char *text ) {	
	if ( entryCount == 0 ) {
		textLength = &textLengths[1];
		textBuf = textBuffers[1];
	}
...
void idCmdSystemLocal::InsertCommandText( const char *text ) {
	...
	len = strlen( text ) + 1;
	if ( len + *textLength > MAX_CMD_BUFFER ) {
...
void idCmdSystemLocal::AppendCommandText( const char *text ) {
	...
	if ( *textLength + l >= MAX_CMD_BUFFER ) {
...
============
*/
void idCmdSystemLocal::ExecuteCommandBuffer( bool allowWait ) {
	int			i;
	char *		text;
	int			quotes;
	idCmdArgs	args;
...
============
*/
void idCmdSystemLocal::ArgCompletion_DeclName( const idCmdArgs &args, void(*callback)( const char *s ), const char *typeName ) {
	int i, num;

	if ( declManager == NULL ) {
		return;
	}
	idDeclTypeInterface *declType = declManager->GetDeclType( typeName );
	if ( declType == NULL ) {
		return;
	}
	...
	num = declType->Num();
	for ( i = 0; i < num; i++ ) {
		callback( idStr( args.Argv( 0 ) ) + " " + declType->FindByIndex( i, false )->GetName() );
	}
}
...
/*
============
idCmdSystem::ArgCompletion_StartGame

The map/rules completion data is game-owned in ETQW.
============
*/
void idCmdSystem::ArgCompletion_StartGame( const idCmdArgs &args, argCompletionCallback_t callback ) {
	if ( game != NULL ) {
		game->ArgCompletion_StartGame( args, callback );
	}
}
