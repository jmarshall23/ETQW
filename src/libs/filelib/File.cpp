...
int idFile::Read1DFloatArray( float* dst ) {
	int count = 0;
	int result = ReadInt( count );
	if ( count > 0 && dst != NULL ) {
		result += ReadFloatArray( dst, count );
	}
	return result;
}

int idFile::ReadFloatArray( float* dst, const int num ) {
	if ( dst == NULL || num <= 0 ) {
		return 0;
	}
	const int result = Read( dst, sizeof( float ) * num );
	LittleRevBytes( dst, sizeof( float ), num );
	return result;
}
...
int idFile::WriteDouble( const double value ) {
	...
	LittleRevBytes( &converted, sizeof( converted ), 1 );
	return Write( &converted, sizeof( converted ) );
}
...
int idFile::WriteCQuat( const idCQuat &quat ) {
	idCQuat value = quat;
	LittleRevBytes( &value, sizeof( float ), sizeof( value ) / sizeof( float ) );
	return Write( &value, sizeof( value ) );
}

int idFile::WriteAngles( const idAngles &angles ) {
	idAngles value = angles;
	LittleRevBytes( &value, sizeof( float ), sizeof( value ) / sizeof( float ) );
	return Write( &value, sizeof( value ) );
}
...
int idFile::Write1DFloatArray( const int num, const float* src ) {
	int result = WriteInt( num );
	if ( num > 0 && src != NULL ) {
		result += WriteFloatArray( src, num );
	}
	return result;
}
...
int idFile::WriteFloatArray( const float* src, const int num ) {
	...
	float* converted = static_cast< float* >( _alloca( sizeof( float ) * num ) );
	memcpy( converted, src, sizeof( float ) * num );
	LittleRevBytes( converted, sizeof( float ), num );
	return Write( converted, sizeof( float ) * num );
}
...
=================
*/
int idFile_Memory::Length( void ) const {
	return fileSize;
}
...
void idFile_Memory::MakeWritable( void ) {
	if ( allocated == 0 && fileSize > 0 ) {
		const int oldOffset = curPtr != NULL && filePtr != NULL ? static_cast< int >( curPtr - filePtr ) : 0;
		allocated = fileSize + granularity;
		char* writable = static_cast< char* >( Mem_Alloc( allocated ) );
		memcpy( writable, filePtr, fileSize );
		filePtr = writable;
		curPtr = filePtr + Min( oldOffset, fileSize );
	}
	mode = ( 1 << FS_WRITE );
	maxSize = 0;
}
...
=================
*/
int idFile_BitMsg::Length( void ) const {
	return msg->GetSize();
}
...
================
*/
int idFile_Permanent::Length( void ) const {
	return fileSize;
}
...
void idFile_Buffered::SeekInternal( long offset ) {
	if ( source != NULL ) {
		source->Seek( offset, FS_SEEK_SET );
	}
}
