...
Adds a localizable string and returns its generated string-table key.  Existing
values are reused so repeated localization passes remain stable.
============
*/
const char* idLangDict::AddString( const wchar_t* str ) {
	...
	for ( int i = 0; i < args.Num(); i++ ) {
		if ( args[ i ].value.Cmp( str ) == 0 ) {
			return args[ i ].key.c_str();
		}
	}

	idLangKeyValue kv;
	kv.key = va( "#str_%08i", GetNextId() );
	kv.value = str;
	const int index = args.Append( kv );
	hash.Add( GetHashKey( args[ index ].key ), index );
	return args[ index ].key.c_str();
}
...
bool idLangDict::ExcludeString( const wchar_t* str ) const {
	...
	const int length = idWStr::Length( str );
	if ( length <= 1 ) {
		return true;
	}
	if ( idWStr::Cmpn( str, L"#str_", STRTABLE_ID_LENGTH ) == 0 ) {
		return true;
	}
	if ( idWStr::Icmpn( str, L"gui::", 5 ) == 0 || str[ 0 ] == L'$' ) {
		return true;
	}
...
int idLangDict::GetNextId() const {
	...
	for ( int i = 0; i < args.Num(); i++ ) {
		...
		if ( idStr::Cmpn( key, STRTABLE_ID, STRTABLE_ID_LENGTH ) != 0 ) {
			continue;
		}
		const int value = atoi( key + STRTABLE_ID_LENGTH );
		if ( value > next ) {
			next = value;
		}
	}
	return args.Num() == 0 ? next : next + 1;
}
